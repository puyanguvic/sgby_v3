...
- `scenes/Main.tscn`: 第七阶段主场景（流程化菜单 + HUD + 启动自检 + 运行态）
- `scripts/BridgeHost.cs`: 引擎桥接宿主
- `scripts/EngineViewport.cs`: 画面显示与输入映射
- `scripts/ShellUI.cs`: 顶栏/侧栏/快捷按钮控制
- `scripts/BridgeNative.cs`: `ibaye_godot_bridge` P/Invoke（含运行态读取）
- `scripts/CityPanel.cs`: 时期切换 + 城市列表 + 城市详情
- `scripts/MainMenuPanel.cs`: Godot 原生主菜单（先选时期再启动）
- `scripts/BattleHud.cs`: 战斗 HUD（节奏、警报、最近指令）
- `scripts/BootGuardPanel.cs`: 启动自检与自动修复面板
- `scripts/IBayeBridge.cs`: 第一阶段最小脚本（保留兼容）
